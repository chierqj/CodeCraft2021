...
class Server:
    ...
    def add_2nodes(self, vir, vir_id, day, svr_id):
        for node in self.nodes:
            ...
            if not flag:
                print(
                    "部署超出容量: day: {}  vir_id: {}  svr_id: {}  server_nodes: A[{}, {}] B[{}, {}]"
                    .format(day, vir_id, svr_id, self.nodes[0].cpu,
                            self.nodes[0].memory, self.nodes[1].cpu,
                            self.nodes[1].memory))
                exit()
        self.working = 1
        self.num_virs += 1
    ...
    def show_info(self, rank='null', node='ALL', save_path='./'):
        ...
        if node == "ALL":
            ...
            plt.plot(xx,
                     self.nodes[0].cpu_rate_per_day,
                     color='green',
                     label='A_cpu_rate_per_day')
            plt.plot(xx,
                     self.nodes[0].memory_rate_per_day,
                     color='sienna',
                     label='A_memory_rate_per_day')
            plt.plot(xx,
                     self.nodes[1].cpu_rate_per_day,
                     color='purple',
                     label='B_cpu_rate_per_day')
            plt.plot(xx,
                     self.nodes[1].memory_rate_per_day,
                     color='black',
                     label='B_memory_rate_per_day')
            plt.legend()
            plt.title("server--cost-rank--" + str(rank) + "--" + self.name +
                      "--" + node)
            plt.xlabel("day")
            # plt.show()
            plt.savefig(save_path + '/cost-rank-' + str(rank) +
                        "_working_status.jpg")
            plt.clf()
...
class Judge:
        ...
        for i in range(t):  # per_day
            ...
            if opt == "purchase":
                ...
                for j in range(int(pur_num)):
                    svr_name, buy_num = f.readline().strip()[1:-1].split(
                        sep=', ')
                    tmp_pur_num += int(buy_num)
                    for _ in range(int(buy_num)):
                        self.svr_pool.append(deepcopy(
                            self.m_servers[svr_name]))
                        tmp_hardware_cost += self.svr_pool[-1].hardware_cost
                        self.svr_pool[-1].true_cost += self.svr_pool[
                            -1].hardware_cost
            ...
            # if int(mig_num) > (3 * len(self.vir_pool) // 100):
            # print("迁移数量超出限制")
            if opt == "migration":
            ...
            for req in self.m_requests[i]:
                if req.type_req == "ADD":
                    ...
                    if vir.node == 1:
                        ...
                        svr_id, node = f.readline().strip()[1:-1].split(
                            sep=', ')
                        svr_4_vir = int(svr_id)
            ...
            if VIEW:
                self.servers_working_rate_per_day.append(working_num /
                                                         len(self.svr_pool))
                self.servers_cpu_rate_per_day.append(1 - tmp_cpu_occupy /
                                                     tmp_cpu_resource)
                self.servers_memory_rate_per_day.append(1 - tmp_memory_occupy /
                                                        tmp_memory_resource)
                ...
                with open(servers_info_per_day_statistics_path,
                          'a',
                          encoding='UTF-8') as sf:
                    sf.write(
                        'day[{}] 服务器总数量:   {}'.format(i, len(self.svr_pool)) +
                        '\n')
                    sf.write('day[{}] 服务器开机数量:   {}'.format(i, working_num) +
                             '\n')
                    sf.write('day[{}] 服务器关机数量:   {}'.format(
                        i,
                        len(self.svr_pool) - working_num) + '\n')
                    sf.write('day[{}] 针对所有开机服务器统计:   '.format(i) + '\n')
                    sf.write('day[{}] 平均部署虚拟机数量:   {}'.format(i, nv_mean) +
                             '\n')
                    sf.write('day[{}] 部署虚拟机数量总数:   {}'.format(i, nv_sum) +
                             '\n')
                    sf.write('day[{}] 部署虚拟机数量方差:   {}'.format(i, nv_var) +
                             '\n')
                    sf.write('day[{}] 部署虚拟机数量中位数:  {}'.format(i, nv_median) +
                             '\n')
                    sf.write('day[{}] 部署虚拟机数量最大值:  {}'.format(i, nv_max) +
                             '\n')
                    sf.write('day[{}] 部署虚拟机数量最小值:  {}'.format(i, nv_min) +
                             '\n')
                    sf.write('day[{}] 购买成本:  {}'.format(
                        i, self.hardware_cost_per_day[-1]) + '\n')
                    sf.write('day[{}] 电费:  {}'.format(
                        i, self.energy_cost_per_day[-1]) + '\n')
                    sf.write('day[{}] 购买数量:  {}'.format(
                        i, self.pur_num_per_day[-1]) + '\n')
                    sf.write('--------------------------------------------\n')
    ...
    def get_statistics_info(lis):
        arr = np.array(lis)
        return np.mean(arr), np.sum(arr), np.var(arr), np.median(arr), np.min(
            arr), np.max(arr)
    ...
    def show_scores(self):
        ...
        with open(
                self.log_path + '/' + self.input_data.split('/')[1][:-4] +
                '_scores.txt', 'w') as f:
    ...
    def plot_working_status(self):
        xx = [i + 1 for i in range(len(self.m_requests))]
        plt.plot(xx,
                 self.servers_working_rate_per_day,
                 color='red',
                 label='total_servers_working_rate')
        plt.plot(xx,
                 self.servers_cpu_rate_per_day,
                 color='green',
                 label='total_servers_cpu_rate')
        plt.plot(xx,
                 self.servers_memory_rate_per_day,
                 color='blue',
    ...
    def plot_topK_cost_svr(self, K, node="ALL"):
        ...
        if not os.path.exists(save_path):
            os.mkdir(save_path)
        sorted_svr_pool = sorted(self.svr_pool,
                                 key=lambda x: x.true_cost,
                                 reverse=True)
